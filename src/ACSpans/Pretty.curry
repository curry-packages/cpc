...
defaultOptions =
  Options { pageWidth        = 78
          , indentationWidth = 2
          , qualification    = None -- IMPORTS and qualified ms unten
          , moduleName       = ""
          , showLocalSigs    = False
...
ppCExpr' _ opts (CVar     _ pvar) = ppCVarIName opts pvar
ppCExpr' _ opts (CLit     _ lit ) = ppCLiteral opts lit
ppCExpr' _ opts (CSymbol  _ qn  ) = ppQFuncParsIfInfix opts qn
ppCExpr' p opts app@(CApply _ f exp)
    | isITE app
        = parensIf (p > tlPrec)
...
ppCExpr' p opts (CLambda _ ps exp) =
...
ppCExpr' p opts (CDoExpr _ stms) =
  ...
  $ text "do" <+> align (vvsep $ zipWith ppCStatement statOptsList stms)
    where statOptsList = optsWithIncreasingNamespaces
                            ...
                            stms
                            opts
ppCExpr' _ opts (CListComp _ exp stms) =
